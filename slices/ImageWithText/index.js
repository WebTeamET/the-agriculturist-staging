  ...
  const components = {
    ...
    paragraph: ({ children }) => (
      <p
        ...
        style={textStyle}
      >
        {tagline && slice?.variation === "imageWithTaglineText" && (
          <span className="text-dark_gray">{tagline}, </span>
        )}
        {children}
      </p>
    ),
    ...
    <Fragment>
      ...
      >
        <div className="w-full relative grid grid-cols-1 lgscreen:grid-cols-2 items-center lgscreen:gap-[10px] gap-30 rounded-[20px]">
          <div
            className="absolute right-[960px] top-1 h-full w-[2px] border-r border-dashed border-green transform origin-top-rightrotate-[17deg] z-[99]"
          ></div>
          {image_position === "Left" && (
      ...
      ) : slice?.variation === "imageWithTaglineText" ? (
        <section
          className="container-lg py-100 lgscreen2:pb-40 lgscreen2:pt-01"
          style={{ backgroundColor: background_color || "white" }}
        >
          <div className="w-full relative grid grid-cols-1 lgscreen:grid-cols-2 items-center lgscreen:gap-[10px] gap-30 rounded-[20px]">
            ...
            >
              ...
              <span className="text-green">{tagline},</span>
              <PrismicRichText
                components={components}
                field={description}
                style={textStyle}
